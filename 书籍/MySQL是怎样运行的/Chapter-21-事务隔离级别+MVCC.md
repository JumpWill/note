...
&emsp;&emsp;随着系统的运行，在确定系统中包含最早产生的那个`ReadView`的事务不会再访问某些`update undo日志`以及被打了删除标记的记录后，有一个后台运行的`purge线程`会把它们真正的删除掉。关于更多的purge细节，我们将放到纸质书中进行详细介绍，不见不散～
  ...
  [24-01]: ../imgs/MySQL是怎样运行的/24-01.png
  [24-02]: ../imgs/MySQL是怎样运行的/24-02.png
  [24-03]: ../imgs/MySQL是怎样运行的/24-03.png
  [24-04]: ../imgs/MySQL是怎样运行的/24-04.png
  [24-05]: ../imgs/MySQL是怎样运行的/24-05.png
  [24-06]: ../imgs/MySQL是怎样运行的/24-06.png
  [24-07]: ../imgs/MySQL是怎样运行的/24-07.png
  [24-08]: ../imgs/MySQL是怎样运行的/24-08.png
  [24-09]: ../imgs/MySQL是怎样运行的/24-09.png
  [24-10]: ../imgs/MySQL是怎样运行的/24-10.png
  [24-11]: ../imgs/MySQL是怎样运行的/24-11.png
...
A事务修改了x值,B事务读取了A修改的x值,A回滚了,B读的数据X就是错的.

#### 不可重复读

A读去X值,B修改了X值,A再次读取X值,A前后读取B修改的X,前后不一致.

#### 幻读

A事务以条件C读取了a条数据,B事务新增了b条数据(满足条件C),A再次读取得到了a+b条数据.
...
每一个对于某一条数据的修改,会形成一个版本链表
![][24-07]
不同事务对于这个链表的数据,会根据事务的id,进行可见区分,从而到达MVCC

#### ReadView

设计`InnoDB`的大佬提出了一个`ReadView`的概念，这个`ReadView`中主要包含4个比较重要的内容：
...
1. 访问的trx_id与ReadView的creator_trx_id相同

         我自己修改的数据---->能看

2. 访问的trx_id小于ReadView的min_trx_id

        在我读之前已经完成提交的数据---->能看

3. 访问的trx_id大于ReadView的max_trx_id

         我读之后新进来的事务----->不能看

4. 访问的trx_id在min_trx_id和max_trx_id之间

        如果trx_id存在于m_ids中，则该记录不可见，事务正在活跃未提交
        如果trx_id不存在于m_ids中，则该记录可见，事务正在活跃未提交

##### 不同隔离级别的ReadView

###### READ COMMITTED

每次读取数据都会生成ReadView,在前后两次读中
ReadView数据不一致,
例如事务A第一次读的时候,B事务正在活跃,生成的ReadView的活跃ID有B,A读不到B修改的数据,
B提交后,A又读,生成的ReadView的活跃ID没有B,A能看到B修改的数据.

###### REPEATABLE READ

第一次读取数据都会生成ReadView,以后一直用它.
前后两次读的时候,B在中途提交了,ReadView的活跃ID有B,A读不到B修改的数据.

在 RR 隔离级别中存在两种读操作：

1. 快照读：数据库中一种读取数据的方式，它基于事务开始时的一个一致性快照来读取数据。快照读可以提供事务开始时的数据视图，即使在事务执行期间其他事务对数据进行了修改，也不会影响快照读取到的数据。简单理解，快照读就是事务开启时创建一个缓存，之后的查询都会从这个缓存中获取数据。即一直使用最开始生成的ReadView
2. 当前读：数据库中一种读取数据的方式，它读取最新提交的数据，而不是基于事务开始时的一致性快照。又生成了一个ReadView,而不是使用最开始生成的那个.

#### undo日志的删除

##### insert undo日志的删除

insert undo日志在事务提交之后就可以删除了,因为insert undo日志只记录了事务修改的记录,没有记录事务修改的记录之前的版本,所以insert undo日志在事务提交之后就可以删除了.

##### update undo日志的删除

update undo日志是为了支持MVCC而存在的,如果系统中最早生成的ReadView的事务值为trx_num,那么
小于该trx_num的update undo日志都可以删除了.因为该数据对所有新的事务都是可见的.

删除update undo日志的过程被称为purge.
