...
![avatar](../imgs/MySQL是怎样运行的/10-01.png)
...
![avatar](../imgs/MySQL是怎样运行的/10-02.png)
...
![avatar](../imgs/MySQL是怎样运行的/10-03.png)
...
![avatar](../imgs/MySQL是怎样运行的/10-04.png)
...
![avatar](../imgs/MySQL是怎样运行的/10-05.png)
...
![avatar](../imgs/MySQL是怎样运行的/10-06.png)
...
![avatar](../imgs/MySQL是怎样运行的/10-07.png)
...
使用二级索引查找的列有可能为null,找到主键,然后回表查询
...
这种访问表的方式的查询是一个范围,使用二级索引查找,找到主键,然后回表查询.
...
即使用到了两个二级索引,得到了id值,且条件时and,则需要将两个索引得到的id求交集(快慢指针).
...
即使用到了两个二级索引,得到了id值,且条件时or,且都是得到的id时由小到大,则需要将两个索引得到的id求并集.
...
即使用到了两个二级索引,得到了id值,且条件时or,一个是由小到大而一个是由大到小,则需要将其中一个的id排序,然后求并集.

```sql
# key1 得到的是由大到小,key3 得到的是由小到大
SELECT * FROM single_table WHERE key1 < 'a' OR key3 > 'z'
```
